...
class digital_lock{
    public static void main(String args[]){
        ...
        possible[1][9]=possible[9][1]=possible[2][8]=possible[8][2]=possible[3][7]=possible[7][3]=5;
        possible[4][6]=possible[6][4]=5;
        boolean[] visited=new boolean[10];
        int ans=0;
        for(int i=m;i<=n;++i){
            ans+=dfs(visited,possible,1,i-1)*4;
            ans+=dfs(visited,possible,2,i-1)*4;
            ans+=dfs(visited,possible,5,i-1)*1;
        }
        System.out.println(ans);
    }
    public static int dfs(boolean[] visited,int[][] possible,int num,int rem){
        if(rem<0) return 0;
        if(rem==0) return 1;
        visited[num]=true;
        int ans=0;
        for(int i=1;i<=9;++i){
            if(!visited[i] && (possible[num][i]==0 || visited[possible[num][i]])){
                ans+=dfs(visited,possible,i,rem-1);
            }
        }
        visited[num]=false;
        return ans;
